...
long oracle(long msg){	   			   	/* takes 64 bit msg, returns 64 bit cipher text */
	long k0 = 0x0123456789abcdef, k1 = 0xaaaabbbbccccdddd,
...
long dec_oracle(long msg){	   			   	/* takes 64 bit msg, returns 64 bit cipher text */
	long k0 = 0x0123456789abcdef, k1 = 0xaaaabbbbccccdddd,
	rc[12] = {0x0000000000000000,0x13198a2e03707344,0xa4093822299f31d0,0x082efa98ec4e6c89,0x452821e638d01377,0xbe5466cf34e90c6c,0x7ef84f78fd955cb1,0x7aacf4538d971a60,0xc882d32f25323c54,0x9b8ded979cd838c7,0xd3b5a399ca0c2399,0x3f84d5b5b5470917};  
	
	long temp_k0, alpha = 0xc0ac29b7c97c50dd, beta = rc[11];	temp_k0 = k0;
	
	k0 = k1^beta, k1 = temp_k0^alpha;
	long new_k0 = k0 ^ alpha ^ beta;
	long new_k1 = k1 ^ alpha ^ beta;
	
	for(int i=0;i<11;i++){
		if (i<=5)
			msg = (i%2 == 0) ? msg^k0^rc[i] : msg^k1^rc[i];
		else
			msg = (i%2 == 0) ? msg^new_k0^rc[i] : msg^new_k1^rc[i];
		
		if(i<5)
			msg = SR(MC(SB(msg)));
		else if(i == 5)
			msg = inv_SB(rc[11]^new_k1^MC(k0^SB(msg)));
			
		else
			msg = inv_SB(MC(inv_SR(msg)));
			
		}

	msg = msg^new_k1^rc[11];
