...
void add_twk(uint64_t *msg, uint64_t *key){	
    msg[0] ^= key[0]&0xffffffff00000000;
...
void generate_twk_key(uint64_t *key, uint64_t *twk_key){
...
void oracle(uint64_t *msg, uint64_t *original_key){
    ...
    uint64_t *twk_key = mem_alloc(64*NO_OF_ROUNDS);
